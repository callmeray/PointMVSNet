...
class ImageFeatureConv(nn.Module):
    def __init__(self, in_channels):
        super(ImageFeatureConv, self).__init__()
        self.in_channels = in_channels
        self.conv = nn.Sequential(
            Conv2d(in_channels, in_channels, (2, 1), padding=(1, 0), groups=in_channels),
            Conv2d(in_channels, in_channels, (2, 1), padding=0, groups=in_channels),
...
class MAELoss(nn.Module):
    def forward(self, pred_depth_image, gt_depth_image, depth_interval):
        ...
        masked_mae = torch.sum((masked_mae / (depth_interval + 1e-7)) / denom)
